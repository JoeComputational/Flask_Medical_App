...
class PostgreSQL():
    ...
    @classmethod
    def get_driver(cls, host, db, user, passwd, port=5432):
        return PostgreSQL(host, db, user, passwd, port)

    def __init__(self, host, db, user, passwd, port=5432):
        self._host = host
        self._db = db
        self._user = user
        self._passwd = passwd
    ...
    def connect_db(self):
        ...
        try:
            connection = psycopg2.connect(host=self._host, database=self._db,
    ...
    def execute_update(self, query):
        ...
        if connection is None:
            Logger.error(
                "Can't update data because no connection is available")
            return False
    ...
    def execute(self, query):
        ...
        if connection is None:
            Logger.error(
                "Can't Execute Query Because No Connection Is Available")
            return False
    ...
    def execute_transection(self, transaction):
        ...
        if connection is None:
            Logger.error(
                "Can't Execute Transaction because no connection is available")
            return False
        flag = False
        cursor = connection.cursor()
        try:
    ...
    def save(self, table, data):
        ...
        try:
            ...
            if type(data) is dict:
                if len(data.keys()) == 1:
                    query = qformat.format(table, ', '.join(
                        data.keys()), f"('{tuple(data.values())[0]}')")
                else:
                    query = qformat.format(table, ', '.join(
                        data.keys()), tuple(data.values()))
